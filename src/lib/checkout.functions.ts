...
export const getDraftOrder = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: order, error } = await supabase
      .from("orders")
      .select("id, status, subtotal_cents, discount_cents, platform_fee_cents, payment_fee_cents, total_cents, payment_method, expires_at, paid_at, buyer_id, order_items(id, title, quantity, unit_price_cents, total_cents, event_id, course_id, ticket_batch_id)")
      .eq("id", data.orderId)
      .maybeSingle();
    ...
    const { data: payment } = await supabase
      .from("payments")
      .select("id, status, amount_cents, provider, pix_qr_code, pix_qr_code_base64, expires_at, updated_at")
