-- PAGOU: bloqueia mutações financeiras diretas e implementa check-in atômico.

-- Pedidos, itens, reservas e logs só podem ser alterados pelo backend confiável.
...
-- Impede autoaprovação de produtores pela API pública. O cadastro será feito
-- posteriormente por função de servidor que sempre inicia com status pending.
REVOKE INSERT, UPDATE, DELETE ON public.seller_accounts FROM authenticated;

-- Códigos de cupom não devem ser enumeráveis no catálogo público.
REVOKE SELECT ON public.coupons FROM anon, authenticated;

-- Progresso e matrícula são atualizados por funções controladas, não por PATCH direto.
REVOKE INSERT, UPDATE, DELETE ON public.enrollments FROM authenticated;

-- Remove políticas antigas que davam acesso amplo a toda a equipe de check-in.
...
-- Criação transacional do pedido de ingresso. O bloqueio do lote impede
-- overselling quando várias pessoas compram ao mesmo tempo.
...
-- Endurece a entrega: confere valor e estoque novamente antes de emitir.
...
BEGIN
  ...
  -- Trava e valida todos os lotes antes de entregar qualquer item.
